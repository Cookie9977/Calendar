...
public class FriendList extends DefaultComboBoxModel<Friend> {
	...
	public FriendList() {
		String SQL = "SELECT user.id, user.username, user.email  FROM friend_link LEFT JOIN user ON friend_link.reciver = user.id WHERE friend_link.accepted = 1 AND friend_link.requester = "
				+ Storage.id;
